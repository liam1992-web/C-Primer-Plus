...
class Waiter : virtual public Worker //必须使用public来限定每一个基类；除非特别指出，否则编译器将认为是私有派生
{
    private:
        int panache;
    protected:
...
class SingingWaiter : public Singer, public Waiter
{
    ...
    public:
        SingingWaiter() {}
        SingingWaiter(const std::string & s, long n, int p = 0, int v = other) : Worker(s, n), Waiter(s, n, p), Singer(s, n, p) {}
